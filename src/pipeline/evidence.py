...
def build_user_message(
    ...
    *,
    ...
    Args:
        ...
        low_evidence:   的 abstention 信号。为 True 时追加一句提示，
                        让模型倾向于明确说"资料不足"而不是硬编答案。
        web_forced:     用户显式要求联网、系统**已经**执行了联网检索。
                        为 True 时告知模型这一事实。

                        为什么必须告知：实测用户说"请触发网页搜索"后，
                        系统确实联网并召回了正确资料，但模型开口第一句
                        仍是「我**无法主动触发网页搜索** —— 本对话中的
                        检索结果由系统自动附带生成」。它把用户的指令
                        当成了"要求我自己去联网"，于是先做一段能力
                        澄清，再才用资料作答。

                        这不是模型的错：它确实看不到检索是怎么发生的，
                        <evidence> 里也没有任何"这是刚为你联网取的"的
                        标记。缺的是**动作已完成**这个事实的回传。
                        补上之后模型就不再自我辩解，直接给答案。
    ...
    if web_forced:
        # 放在证据之后、问题之前：紧邻 <question>，模型读到指令时
        # 这条事实还在近距离上下文里，不容易被长证据稀释。
        parts.append(
            "<search_note>用户要求联网检索，系统**已经完成**实时网页搜索，"
            "上方 <evidence> 中类型为「实时网页」的资料就是本次联网取回的结果。"
            "因此请直接基于这些资料作答，"
            "**不要**声明自己无法联网或无法触发搜索 —— 检索动作已经由系统执行完毕。"
            "若这些资料仍不足以回答，如实说明缺什么即可。"
            "</search_note>"
        )
    if low_evidence:
