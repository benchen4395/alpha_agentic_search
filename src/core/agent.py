...
from src.core.llm_client import chat as llm_chat, stream_chat as llm_stream_chat
# LLM 预热：把本地 ollama 的模型加载成本移到启动期（见 warmup() 的说明）
from src.core.llm_client import warmup_all as llm_warmup_all
from src.core.memory import ConversationMemory
from src.configs.prompts import PROMPTS, build_summary_system
from src.cache.qa_cache import QACache
from src.pipeline.query_rewriter import query_rewrite_route
# 显式联网指令识别：给启发式 L4 判据一个用户可控的逃生舱
# （为何需要、为何用规则而非 LLM，见 pipeline/web_intent.py 顶部）
from src.pipeline.web_intent import wants_web_search, strip_web_directive
from src.search.searcher import web_search, format_results
from src.pipeline.tool_router import route_and_call, format_tool_result

# ---- L1 缓存准入策略（时效判定 / 分级 TTL / 低质答案识别）----
# hit_admissible：**读取侧**复核，让判据升级能回溯作用于存量条目
# （只挂写入侧时，收紧判据对已在 L1 里的脏条目零作用 —— 见其 docstring）
from src.cache.cache_policy import decide_cacheability, hit_admissible

# ---- 证据清洗与 <doc> 结构化定界（Prompt Injection 防护）----
from src.pipeline.evidence import build_evidence_block, build_user_message

# ---- 结构化答案契约（sources / citations / confidence）----
from src.core.answer_types import (
...
from src.pipeline.followup import FOLLOWUP_MODE, StreamFilter, parse_followups
...
class AgenticSearchAgent:
    def __init__(
        ...
        #
        # ⚠️ 必须用 `is not None` 而**不能**用 `qa_cache or QACache(...)`：
        # QACache 定义了 `__len__`，于是一个**空**缓存的布尔值是 False，
        # `or` 会把它悄悄丢掉、转而新建一个读磁盘的实例。
        # 后果非常隐蔽：注入空 cache 的调用方（单测 / 隔离环境）以为自己
        # 拿到了干净的内存缓存，实际 agent 用的是 data/qa_cache 里那 72 条
        # 存量条目 —— 测试会莫名命中生产数据，且不报任何错。
        self.qa_cache: QACache = qa_cache if qa_cache is not None else QACache(
    ...
    def _prefetch_location() -> None:
        ...
        try:
            ...
            from src.pipeline.context_provider import get_location_context as _geo
    ...
    def _l1_get(self, query: str, namespace: Optional[str]) -> Optional[str]:
        ...
        if hit is None and namespace is not None:
            # 回退：全局公共池（只含与身份无关的公共知识）
            hit = self.qa_cache.get(query, namespace=None)
        if hit is None:
            return None
        # ⚠️ 命中后还要复核一次准入（见 `_l1_admissible`）
        if not self._l1_admissible(query, hit):
            return None
        return hit

    def _l1_admissible(self, query: str, answer: str) -> bool:
        """对**已命中**的 L1 条目再跑一次准入判据（读取侧复核）。

        ════════════════════════════════════════════════════════════════
        为什么写入侧的门禁不够 —— 「判据升级不回溯」
        ════════════════════════════════════════════════════════════════
        `decide_cacheability()` 一直只挂在**写入**路径上
        （`_archive_if_enabled`）。这意味着它的语义是
        「今后不准再写这种条目」，而**不是**「这种条目不准被返回」。
        两者的差距就是**已经躺在 L1 里的存量脏数据**。

        实测（枚举 `QACache._mem` 共 72 条）：按**当前**判据本应被拒、
        却仍能被毫秒命中的有 7 条，其中包括用户真实遇到的那条：
        ...
        用户的困惑正来自这里：他刚刚才让系统联网查到「法国 18 岁」这个
        正确事实并沉淀进 L3，下一轮再问「美国…法国和日本呢」时却仍旧
        毫秒返回旧的推断式拒答 —— 因为 **L1 短路发生在任何检索之前**，
        新入库的 L3 知识根本没有机会参与。判据虽已收紧，对存量条目零作用。

        ════════════════════════════════════════════════════════════════
        修法：把同一套判据也挂到读取侧
        ════════════════════════════════════════════════════════════════
        命中后用 `decide_cacheability()` 复核；判为不可缓存则**当作 miss**
        继续走完整链路。于是判据的每一次收紧都会**立即**对存量条目生效，
        不需要手工清库、也不需要等 TTL 自然过期（stable 档是 30 天）。

        为什么不在这里顺手删掉该条目：命中可能来自 **fuzzy**（key 是另一
        个近似问句），按 `query` 去删会删错对象或空删。抑制已经足以让它
        不再影响答案；真正的物理清理交给 `scripts/purge_l1_stale.py`。

        代价：每次 L1 命中多一次纯字符串扫描。实测判据全程无 I/O、无模型
        调用，量级在微秒 —— 相对它挡住的「毫秒级返回一个错答案」可以忽略。

        `enable_cache_policy=False`（A/B 对照组）时直接放行，保持旧行为。

        ⚠️ 判据本体在 `cache_policy.hit_admissible()`，这里只做"是否启用 +
        日志"。原因见那个函数的注释：L1 有多个读取入口（agent 短路 /
        retriever 第一层 / RRF 融合的一路），判据必须是**同一份**实现，
        否则前门拦住、后门放进来。
        """
        if not self.enable_cache_policy:
            return True
        try:
            rejected = hit_admissible(query, answer)
        except Exception as e:
            # 判据本身出错不该让缓存整体失效（那会让每次查询都退化成全链路）
            print(f"[agent] L1 读取侧复核异常，放行本次命中: {e}")
            return True
        if rejected is None:
            return True
        print(
            f"[agent] L1 命中被读取侧复核拒绝 (tier={rejected.tier}): "
            f"{rejected.reason} → 当作未命中，继续走完整检索"
        )
        return False
    ...
    def chat(
        ...
        # ══════════════════════════════════════════════════════════════════
        # 显式联网指令识别（必须在 L1 短路**之前**）
        # ══════════════════════════════════════════════════════════════════
        # 用户说"请联网搜一下"时，其诉求是"拿到新的、来自网络的资料"。
        # 若这时还先查 L1，就可能直接把**上一次拒答的缓存结果**毫秒返回
        # —— 用户明确要求重新搜，却拿到一字不改的旧答案，是比不联网
        # 更难理解的行为。所以这里将 L1 一并旁路。
        #
        # ⚠️ 用**原始输入**判定，不能用改写后的 query：改写器会重写措辞、
        # 把"请触发网页搜索"这类指令性表达丢掉（实测用户 case 里改写
        # 结果就只剩"法国 最低饮酒年龄 法定年龄"，指令词已经消失）。
        # 让手段反过来改变意图判定就是抽象泄漏，同理于 route_query。
        force_web = wants_web_search(user_input)
        # 检索用的 query：剥掉"请你触发网页搜索…的资料搜索"这些指令性废词，
        # 否则它们会参与向量编码、把语义中心从"法国饮酒年龄"稀释掉。
        search_input = strip_web_directive(user_input) if force_web else user_input
        if force_web and verbose:
            print(f"[0/3] 检测到显式联网指令 → 强制激活 L4；"
                  f"检索用 query = {search_input!r}")
        if force_web:
            _emit("web_intent", "显式联网指令",
                  f"已强制激活 L4 web 并旁路 L1 缓存；检索内容：{search_input}")
        ...
        cached = None if force_web else self._l1_get(user_input, namespace)
        ...
        if used_tool and tool_decision is not None:
            ...
            # 用 search_input（已剥掉联网指令）而非原句去改写：
            # 否则"请你触发网页搜索"会被改写器当成提问内容的一部分。
            ...
            if rewritten and rewritten.upper() != config.NO_SEARCH_SENTINEL:
                if self.retriever is not None:
                    ...
                    rag_result = self.retriever.retrieve(
                        ...
                        route_query=search_input,
                        force_web=force_web,
                    )
                    confidence = rag_result.confidence
                    # ════════════════════════════════════════════════════
                    # 这行曾经被误注释掉，導致 abstention 信号完全失效
                    # ════════════════════════════════════════════════════
                    # 原先写成：
                    #   confidence = rag_result.confidence   # low_evidence = rag_result.low_evidence
                    # —— 赋值被并到行尾注释里了，于是 `low_evidence` 永远保持
                    # 初始值 False。后果是一条**完整断掉**的信号链：
                    #   ① `build_user_message(low_evidence=False)` 不再附
                    #      <retrieval_note> → summary 拿不到"资料可能不足"的提示；
                    #   ② `AnswerResult.low_evidence` 永远是 False → 前端与下游
                    #      无法区分"证据充分"与"证据不足"；
                    #   ③ `_emit("retrieve")` 的 detail 里那句 "⚠️ 证据不足"
                    #      永远不会出现 → 连可观测性也一并丢了。
                    #
                    # 为何难以发现：这是一个**静默降级** —— 没有报错、没有
                    # 日志，只是拒答引导失效；而它又恰好发生在"置信度很高但
                    # 资料其实不含答案"的场景，表现为置信度 0.98 却回答
                    # "资料不全"（模型自己看出来的）—— 看起来像是置信度算错，
                    # 实际是拒答信号根本没传下去。
                    low_evidence = rag_result.low_evidence
        ...
        if evidence_block:
            ...
            user_msg = build_user_message(
                ...
                # 告知模型"联网这个动作已经做完了"。不加这句时实测模型会
                # 先声明"我无法主动触发网页搜索"再作答 —— 它把用户的指令
                # 理解成"要求我自己去联网"，而它确实看不到检索是怎么发生的。
        ...
        def _archive_traced(answer_text: str) -> None:
            ...
            self._archive_if_enabled(
                ...
                # 显式联网的结果不得写 L1。原因见 _archive_if_enabled
                # 里 skip_l1 的说明：不阻止的话，“请联网搜一下 X”这句话
                # 本身就成了缓存 key，下次同样的指令会被毫秒命中旧结果。
                skip_l1=force_web,
            )
    ...
    def _archive_if_enabled(
        ...
        # ══════════════════════════════════════════════════════════════════
        # skip_l1：显式联网指令的答案**不得进 L1**
        # ══════════════════════════════════════════════════════════════════
        # chat() 里 force_web 只旁路了 L1 的**读**，没有禁止**写**。
        # 实测后果（在 data/qa_cache 里真实捞到这两条）：
        #     key = '请你触发网页搜索进行法国最低饮酒年龄的资料搜索'
        #       → '我无法主动触发网页搜索……'
        #     key = '请你出触发网页搜索进行法国最低饮酒年龄的资料搜索'
        #       → '根据本次联网检索到的实时网页资料，法国最低饮酒年龄为 18 岁[6]…'
        # 这两条 key 本身 `wants_web_search()` 恒为 True —— 也就是说
        # **每次**遇到这句指令都会判为"用户要求重新联网"，但它自己的答案
        # 却躺在 L1 里。虽然 chat() 的读侧被旁路了，retriever 内部的 L1
        # 仍会把它当证据召回；而且这类条目在语义上本就是错的 ——
        # "请帮我重新搜一次"这句话不该有缓存答案。
        #
        # 另一层理由：联网结果天然带时效性。把它按 stable(30 天) 冻结，
        # 等于把"实时"退化成"上个月的实时"，与用户下指令的本意正相反。
        #
        # 注意只跳过 L1，**L3 仍然写**（下面第 2 步）：L3 只是召回的一路、
        # 要重新过 LLM，把这次联网拿到的新事实沉淀进去正是"越用越强"。
        if skip_l1:
            print("[agent] 跳过 L1 缓存 (显式联网指令：结果具时效性，"
                  "且指令句本身不应成为缓存 key)")
