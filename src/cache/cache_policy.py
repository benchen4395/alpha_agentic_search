# cache_policy.py
"""L1 Q&A 缓存的「准入 / 命中」策略层。

════════════════════════════════════════════════════════════════════════
为什么需要这个模块
════════════════════════════════════════════════════════════════════════
若 `agent._archive_if_enabled()` **无条件**把每一轮 (query, answer)
...
#   1. **年份动态生成**：硬编码 "2024","2025","2026" 明年就会失效。
...
_LOW_QUALITY_ANSWER_CUES: tuple[str, ...] = (
    ...
    # ⚠️ 实测补充：真实答案里出现过「我无法给出**完整的历届获奖名单**」，
    # 而原表只有"无法提供/无法回答"，漏掉了"无法给出"这个高频变体
    # —— 于是这条拒答被判为 tier=stable，写进了 L1(30天) 和 L3。
    # 教训：这类线索表必须用**真实产出**去校准，凭直觉列举一定有遗漏。
    "无法给出", "无法列出", "不能给出",
...
#            防线 A-3：**部分拒答**检测（修复②）
# ════════════════════════════════════════════════════════════════════════
# 上面的 `is_low_quality_answer()` 只看**开头 60 字**，这个设计对"纯拒答"
# （"抱歉，我无法回答…"）非常有效，但漏掉了一整类更隐蔽的失败：
#
#   ┌────────────────────────────────────────────────────────────────┐
#   │ **部分拒答**：开头像个好答案，但真正被问的东西没答出来           │
#   └────────────────────────────────────────────────────────────────┘
#
# 实测案例（用户问「茅盾文学奖一共几届、获奖名单是多少」）：
#
#   「根据现有资料，可以确认的信息如下：
#     - 茅盾文学奖由中国作家协会主办，首届于 1982年 颁发 [2,4]。
#     - 评选规则上，参评作品须为13万字以上的长篇小说 [2]。
#     **不足之处**：目前提供的资料没有给出完整的历届届数和获奖名单，
#     无法逐一列出具体获奖作品与作者。建议查阅中国作家协会官方公布的
#     历届获奖名单以获得准确信息。」
#
# 实测 `is_low_quality_answer()` 对它返回 **False** —— 因为前 60 字是
# 「根据现有资料，可以确认的信息如下：茅盾文学奖由中国作家协会主办…」，
# 看起来完全正常。于是它被判为 `tier=stable`：
#     → 写进 L1，**冻结 30 天**
#     → 写进 L3，成为后续检索的"证据"
#
# 这就形成了一个**自我强化的失败循环**（实测已在 L3 召回里观测到）：
#
#     拒答 → 存进 L3 → 下次召回到自己的拒答（calib=0.578）
#          → 置信度虚高 → 不触发 L4 → 再次拒答 → …
#
# 实测证据：查「茅盾文学奖 历届 获奖名单」时，L3 返回的 top-1 就是
#     「历史问答：Q: 新中国成立后,一共有多少届茅盾文学奖…A: 根据现有资料，可以确…」
# —— 系统在拿自己上一次的失败当参考资料。这比单纯答不出来更糟：
# 它会**持续污染**历史层，且越用越严重。
#
# 【为什么不能简单地把检查范围从"前 60 字"扩大到全文】
#   正常的好答案经常在结尾附加免责/延伸说明：
#       "…以上是核心配置。更多细节建议查阅官方文档。"
#       "…（注：数据截至 2023 年，最新情况请以官网为准）"
#   如果直接全文扫 `_LOW_QUALITY_ANSWER_CUES`（含"建议查阅""不清楚"等），
#   这些完全合格的答案会被大量误杀 —— 而误杀的代价是丢掉"越用越强"的积累。
#
# 【解法】另起一套**更严格**的尾部线索表，只匹配那些明确表达
#   "**你问的那个东西我没答出来**"的措辞，而不是泛泛的补充说明。
_PARTIAL_REFUSAL_CUES: tuple[str, ...] = (
    # —— 明确承认"资料里没有" ——
    "没有给出完整", "没有给出具体", "没有完整的", "未给出完整",
    "资料没有给出", "资料中没有", "资料未提供", "现有资料没有",
    "不在现有资料", "不在提供的资料", "超出现有资料",
    "没有直接列出", "未直接列出", "没有具体列出",
    # ⚠️ 实测补充：真实产出里出现过
    #   「完整的历届获奖名单（第一至八届等）在现有资料中未给出」
    # 注意它把"在现有资料中"和"未给出"**倒装**了，所以上面那些
    # 「资料没有给出」的正序组合全都匹配不上。
    # 这类判据必须按真实答案的多种语序来补，不能只写一种句式。
    "中未给出", "中未提供", "里未给出", "未予给出", "均未给出",
    "未写明", "未列明", "未能给出", "未能列出",
    # ⚠️ 实测补充（第三轮，来自「法国最低饮酒年龄」真实 case）：
    #   「根据现有检索资料，**未能直接确认**法国的法定最低饮酒年龄：
    #     …但**并未直接点名法国**…只是基于欧洲通行标准的**推断**」
    # 这条答案既不命中开头检查（前 60 字是"根据现有检索资料…"），
    # 也不命中上面任何尾部线索，于是被判为 tier=stable 写进 L1 30 天。
    # 后果：用户再问同一问题时毫秒命中这条拒答，**永远不会再检索**，
    # 即使后来离线索引更新了或联网能查到也没用。
    # 再一次印证：这类线索表只能靠**真实产出**迭代，凭直觉列不全。
    "未能直接确认", "未能确认", "无法直接确认",
    "并未直接点名", "未直接点名", "没有直接点名",
    # ⚠️ 刻意**不收**「未直接说明 / 没有直接说明」这类：实测会误杀
    #   「…虽然原论文**未直接说明**具体的初始化细节，但完整结构包含…」
    #   —— 这是一条结论完整的好答案，只是顺带交代某条证据的局限。
    #   判据的分界线是：说的是**整个问题**没答出来（拒答），
    #   还是**某条证据**不含某细节（正常的严谨表述）。后者不能拦。
    "并非资料直接", "并非确定结论", "不是确定结论",
    "只是推断", "仅为推断", "属于推断", "的推断，而非",
    "没有新增的", "无新增的", "没有专门针对",
    # —— 明确承认"列不出来 / 答不全" ——
    "无法逐一", "无法一一", "无法完整", "无法列出", "无法给出完整",
    "无法确认具体", "无法提供完整", "不能逐一", "难以完整",
    # —— 明确的自我否定小节标题（模型很爱用这个格式）——
    "不足之处", "局限性说明", "需要说明的是，资料",
    # —— 英文 ——
    "does not provide", "not provided in", "cannot list",
    "unable to list", "not available in the provided",
    "incomplete information",
)

# ── 头部专用子集：只有「开门就明确表态没答出来」的措辞才放进来 ──────────
#
# ⚠️ 不能把整张 `_PARTIAL_REFUSAL_CUES` 拿去扫头部。实测误杀：
#     「Transformer 的核心是自注意力机制。虽然原论文**未直接说明**具体的
#       初始化细节，但完整结构包含…编码器与解码器各堆叠 6 层。」
#   这是一条**结论完整**的好答案，只是顺带如实交代了某条证据的局限。
#   "未直接说明"落在头部 120 字内 → 被误判为部分拒答 → 丢掉缓存积累。
#
# 区别在于：「未能直接确认<被问的东西>」是对**整个问题**的表态，
# 而「某资料未直接说明<某细节>」是对**单条证据**的描述。前者才是拒答。
# 所以头部只收前一类 —— 主语是"我/本次检索"而非"某份资料"的措辞。
_PARTIAL_REFUSAL_HEAD_CUES: tuple[str, ...] = (
    "未能直接确认", "未能确认", "无法直接确认",
    "并未直接点名", "未直接点名", "没有直接点名",
    "并非确定结论", "不是确定结论",
    "没有新增的", "无新增的",
...
# 尾部检查的范围：最后 N 个字符。
# 为什么是"尾部"而不是全文：部分拒答的自我否定**几乎总在结尾**
# （模型先给能给的，最后才交代缺什么）。限定尾部能进一步降低
# 误伤"中间提了一句资料有限、但最终仍给出完整答案"的情况。
# 200 字足以覆盖典型的"不足之处"段落。
_PARTIAL_REFUSAL_TAIL_CHARS: int = int(
    os.getenv("QA_PARTIAL_REFUSAL_TAIL", "200")
)

# 头部检查范围：部分拒答也可能**开门就承认没答出来**，实测
#   「根据现有检索资料，**未能直接确认**法国的法定最低饮酒年龄：…」
# 它在第 12 字之后才表态，超出了 `is_low_quality_answer()` 的 60 字窗口
# 所覆盖的线索表；而结尾又是正常的"建议以官方为准"，尾部扫描也抓不到。
# 给 120 字：足以覆盖"开场表态"，又不会深入正文（避免误杀那些
# 中途如实说明某条证据局限、但最终仍给出完整答案的好答案）。
_PARTIAL_REFUSAL_HEAD_CHARS: int = int(
    os.getenv("QA_PARTIAL_REFUSAL_HEAD", "120")
)


def is_partial_refusal(answer: str) -> bool:
    """判断答案是否为「部分拒答」——开头正常但明确承认没答出核心问题。

    与 `is_low_quality_answer()` 的分工：
        is_low_quality_answer  → 看**开头**，抓"纯拒答"（一上来就说不行）
        is_partial_refusal     → 看**结尾**，抓"部分拒答"（先给边角料，
                                 最后承认核心内容缺失）

    两者是**互补**关系，不是替代：一个答案可能只命中其中一个。

    Args:
        answer: 最终答案文本。
    Returns:
        True 表示这条答案不应进入 L1，**也不应进入 L3**
        （详见 `agent._archive_if_enabled()` 里的说明）。

    示例：
        >>> is_partial_refusal("茅盾文学奖首届于1982年颁发。"
        ...                    "不足之处：资料没有给出完整的获奖名单。")
        True
        >>> is_partial_refusal("配置方法如下：...。更多细节建议查阅官方文档。")
        False
    """
    if not answer:
        return True
    stripped = answer.strip()
    tail = stripped[-_PARTIAL_REFUSAL_TAIL_CHARS:].lower()
    if any(cue in tail for cue in _PARTIAL_REFUSAL_CUES):
        return True
    # ⚠️ 还要扫一次**开头**：实测有一类部分拒答把"没答出来"写在最前面，
    #   「根据现有检索资料，**未能直接确认**法国的法定最低饮酒年龄：…」
    #   然后才列出沾边的信息。它既躲过 `is_low_quality_answer()`
    #   （前 60 字里"未能直接确认"在第 12 字之后、且不在那张线索表里），
    #   又躲过上面的尾部扫描（结尾是"建议以法国现行法律为准"）。
    #   长答案的中间段落不扫 —— 那里出现"未直接说明"往往是在如实描述
    #   某条证据的局限，而结论仍然给全了，扫全文会造成大量误杀。
    #   头部用的是**更窄的子集** `_PARTIAL_REFUSAL_HEAD_CUES`，原因见其注释
    #   （拿全表扫头部实测会误杀"虽然原论文未直接说明…但完整结构包含…"）。
    head = stripped[:_PARTIAL_REFUSAL_HEAD_CHARS].lower()
    return any(cue in head for cue in _PARTIAL_REFUSAL_HEAD_CUES)
...
class CacheDecision:
    ...
    Fields:
        ...
        tier:      命中的策略档位，便于监控统计：
                   ...
                   "reject_partial_refusal" / "reject_too_short" /
                   "web_backed" / "volatile" / "stable"

                   ⚠️ reject_partial_refusal 与 reject_low_quality 分开计：
                   两者的运维含义不同 —— 前者意味着"检索到了一些但不够"
                   （往往是离线索引覆盖不全或需要多跳），后者意味着
                   "完全没检索到"。分开统计才能定位真正的短板。
    """
...
def decide_cacheability(
    ...
    # --- 2.5) **部分拒答**（修复②）---
    # 开头像个好答案、结尾才承认"没给出完整名单"的类型。
    # 它们会绕过上面只看前 60 字的检查，被当成 stable 写进 L1+L3，
    # 然后在 L3 里被自己召回来当"证据"，形成自我强化的失败循环。
    # 完整的实测证据见 `is_partial_refusal()` 上方的大段注释。
    if is_partial_refusal(answer):
        return CacheDecision(
            False, None,
            "答案尾部明确承认核心信息缺失（部分拒答），"
            "写入会在 L3 形成自我强化的失败循环",
            "reject_partial_refusal",
        )
...
# ════════════════════════════════════════════════════════════════════════
#            防线 A'：**读取侧**准入复核（判据升级的回溯机制）
# ════════════════════════════════════════════════════════════════════════
def hit_admissible(query: str, answer: str) -> CacheDecision | None:
    """对**已命中**的 L1 条目复核准入；返回 None 表示放行。

    ════════════════════════════════════════════════════════════════
    为什么写入侧的门禁不够 —— 「判据升级不回溯」
    ════════════════════════════════════════════════════════════════
    `decide_cacheability()` 长期只挂在**写入**路径上，它的语义因此是
    「今后不准再写这种条目」，而**不是**「这种条目不准被返回」。
    两者的差距就是**已经躺在 L1 里的存量脏数据**。

    实测（枚举 `QACache._mem` 共 72 条）：按**当前**判据本应被拒、
    却仍能被毫秒命中的有 7 条：

        [reject_too_short]       你好
        [reject_partial_refusal] 法国最低饮酒年龄
        [reject_partial_refusal] 美国可饮酒的年龄是多少法国和日本呢
        [reject_partial_refusal] 韩国的首都是哪第二大城市是哪一个
        [reject_partial_refusal] 送长辈的热销礼物
        [reject_partial_refusal] 霸王茶姬有哪些产品
        [reject_partial_refusal] 请你触发网页搜索进行法国最低饮酒年龄的资料搜索

    用户的真实困惑正来自这里：他刚让系统联网查到「法国 18 岁」并沉淀进
    L3，下一轮再问「美国…法国和日本呢」时却仍毫秒返回旧的推断式拒答 ——
    因为 **L1 短路发生在任何检索之前**，新入库的 L3 知识根本没机会参与。
    判据虽已收紧，对存量条目零作用。

    ════════════════════════════════════════════════════════════════
    为什么这个函数必须放在本模块（而不是各调用点各写一份）
    ════════════════════════════════════════════════════════════════
    L1 在本系统里有**三个读取入口**：
        ① `agent._l1_get()`            —— Step 0 的毫秒短路
        ② `L1QACacheLayer.lookup()`    —— retriever 内部的第一层
        ③ `L1QACacheLayer.search()`    —— 参与 RRF 融合的一路（复用 ①的 lookup）
    实测只在 ① 加复核时，日志会出现自相矛盾的一幕：

        [agent] L1 命中被读取侧复核拒绝 (tier=reject_partial_refusal)
        …
              RAG 检索完成 [L1_qa:1], 融合 1 段, conf=1.000    ← 同一条拒答又回来了

    也就是 agent 前门拦住了，retriever 后门又放进来，还带着 conf=1.0 的
    最高可信度标签。所以判据必须收敛成**一个**函数，由所有入口共用。

    Args:
        query:  命中条目对应的 query（fuzzy 命中时传当前用户 query 即可，
                判据只用它做时效性判断，两者时效属性一致）。
        answer: 命中到的缓存答案。
    Returns:
        None            → 放行；
        CacheDecision   → 应当拒绝，其中 tier/reason 可直接用于日志。
    """
    decision = decide_cacheability(query, answer)
    return None if decision.cacheable else decision
