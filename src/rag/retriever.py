...
class LayeredRetriever:
    ...
    def retrieve(
        ...
        Args:
            ...
            force_web: 强制激活 L4，**跳过自动兜底判据**。

                用于「用户用自然语言显式要求联网」的场景，由
                `pipeline/web_intent.wants_web_search()` 识别。

                为什么需要这个逃生舱：下面第 4 步的两道判据
                （low_conf / insufficient）都是启发式，存在固有盲区。
                实测 case「法国最低饮酒年龄」——
                  离线召回聚合置信度 0.98、实词覆盖率也达标
                  （"法国""最低""饮酒""年龄" 在维基"合法飲酒年齡"
                    条目里全都出现）
                  → 两道判据都是 False → L4 不触发
                  → 但那篇条目**只讲欧洲通例、没点名法国** → 只能拒答。

                这类"主题高度相关、词面全覆盖，却恰好缺了用户要的那个
                具体事实"的情况，任何基于相似度或词面覆盖的指标都识别
                不了 —— 判断"这段文字里有没有法国的那个数字"本质上需要
                阅读理解。与其无止境调阈值（每次调都会在别处引入新误判），
                不如把最终裁量权交还用户。

                ⚠️ 它**只强制联网，不跳过离线层**：离线证据依然照常
                召回并参与融合。因为用户要的是"补充网络资料"，不是
                "忽略已有资料"；而且离线层只要几十毫秒，没有省的价值。
            namespace: 多租户隔离命名空间。
        ...
        # ---------- 1) L1 短路 ----------
        # force_web 时必须跳过 L1：用户要的是"重新联网取新资料"，
        # 而 L1 里存的恰恰可能是上一轮基于旧证据的**拒答**。实测漏掉这里
        # 会出现最讽刺的一幕 —— 用户显式要求联网，retriever 却毫秒级把
        # 上次那句"没找到法国的规定"当成命中证据返回，L4 根本没机会跑，
        # 日志显示 `[L1_qa:1], 融合 1 段`。
        #
        # ⚠️ agent 层已经旁路过一次 L1（见 agent.chat 的 force_web 分支），
        # 但**这里是另一条独立的短路路径** —— L1 在本系统里既是 agent 的
        # 前置缓存，又是 retriever 的第一层，两处都要处理。只堵一处
        # 等于没堵，这类"同一机制有多个入口"的疏漏正是最容易漏改的。
        if self.l1 is not None and not force_web:
        ...
        # force_web 放在最前面参与 or：用户的显式指令优先于任何启发式。
        # 这样即使 conf 高、覆盖率达标（自动判据都说"不用联网"），
        # 只要用户开口要求，就一定会联网。
        need_web = force_web or low_conf or insufficient
