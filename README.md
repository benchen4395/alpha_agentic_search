...
| **KG 实体消歧** | L5 用 `weight × log1p(入度)` 组合分排序，把「北京→美国小镇」这类同名碰撞压下去；再叠 description 词面重叠与类型-意图匹配两层上下文信号（消歧 6/11 → 11/11）|
...
抄词性误匹配挡在外面。实现与标定过程见 [`src/rag/README.md`](src/rag/README.md) §5.5。

#### 上下文消歧：让 `query_context` 真的起作用

上面的组合分只依赖实体自身的先验，**与 query 无关**。于是同一个 mention 在
语义相反的两个上下文里会拿到完全一样的结果和分数：

```
"华盛顿是美国第一任总统"  → 華盛頓哥倫比亞特區(0.18)   ✗
"美国首都华盛顿的人口"    → 華盛頓哥倫比亞特區(0.18)   ✓  （碰巧对）
```

根因是 cosine 只对命中**热门实体向量库**的候选计算，而那个库只有 2871 条，
对比 KG 的 1038 万实体覆盖率约 0.03%——绝大多数候选走的是与上下文无关的
先验分支。补两层与 query 相关的信号：

| 信号 | 做法 | 效果 |
|---|---|---|
| **description 词面重叠** | 取 description 与 query 的字符 bigram 交集，以 description 侧为分母（精确率语义） | 6/11 → 10/11 |
| **类型-意图匹配** | `P31→Q5` 判定候选是否为人物；query 侧用谓词线索（「提出/发明/担任」vs「位于/单位/人口」）判断问的是人还是物 | 10/11 → **11/11** |

两处反直觉但可复现的结论：

- **短文本上词面匹配优于语义向量。** 给冷门候选现算 description embedding
  只有 4/9 且 673ms/题，反而低于什么都不做的 5/9。原因是 description 极短
  （「磁感應單位強度」7 字、「车型」2 字），BGE-M3 在这个长度上不稳定，把
  「特斯拉工廠」这类**字面接近但语义错误**的候选拉了上来。词面方案 8/9、~0ms。
- **不能用「候选自身有 P279」判断它是不是类实体。** 概念天然是某类的子类，
  这么判会误杀「量子纠缠」「相对论」。只能用**入度**（有多少实体声明"我是它的实例"）。

类型信号的关键设计是**无线索时完全不介入**：query 里没有任何谓词线索时
类型分恒为 0，排序与不启用时完全一致。实测 11 个 case 有 5 个属于这种情况
（「水星是太阳系最内侧的行星」等），它们本来就排对了——这从结构上保证了
**只可能修好、不可能弄坏**。
...
> 实测 `cd /tmp && python -m pytest <repo>/tests` 与在仓库根跑结果一致（均 466 passed）。
...
- [x] **L5 泛实体挤占预算**：混淆式多跳 query 抽出的 7 个 mention 里 6 个是「地方」「科学家」这类类实体，而 `query_kg_end_to_end` 按 mention 顺序消耗 `max_entities` 额度，唯一的真实体「欧洲」还没轮到就被硬 `break` 掉了——**真正的杀手是预算挤占，不是排序噪声**。用 P31/P279 入度识别类实体并重排（默认只重排不丢弃），配合封顶计数把延迟从 349ms 压到 0.16ms
- [x] **L5 上下文消歧失效**：`query_context` 传了但对冷门候选完全没起作用（覆盖率 0.03%）。补 description 词面重叠 + 类型-意图匹配两层信号，消歧 6/11 → 11/11。详见 §4.6
- [ ] **L5 在混淆式多跳题上的贡献**：上面两项修复后 BrowseComp-ZH 准确率仍为 0——该数据集**故意混淆实体名**，题干里客观上没有具名实体可抽（实测 12/12 题实词覆盖率 0.07~0.35，全部走 L4 联网兜底）。这不是 L5 的算法缺陷而是任务性质决定的上限，需靠 loop agent 逐跳把混淆描述还原成实体名，L5 才有介入余地
- [ ] **实体别名覆盖缺口**：部分实体只挂全名不挂中文简称（`蒂姆·库克` 有、`库克` 没有）。已验证否决了「无人物候选时 FTS 简称回退」的思路——触发条件不成立（「苹果」「长城」「欧洲」本就该链到非人物，却会全部误触发），且 `popularity` 是 KG 内部入度而非真实知名度（Tim Cook=1 < 探险家詹姆斯·库克=15），排序层无解。正解是离线引入外部知名度信号重算 popularity
